...
void
leer_directorios(DIR *directory,
                 char *path,
                 char *cadena_a_buscar,
                 char *(*funcion_a_comparar)(const char *, const char *) )
{
	struct dirent *entry;
	...
	while ((entry = readdir(directory)) != NULL) {
		...
		if (entry->d_type == DT_DIR) {
			...
			int subdirectory = openat(dirfd(directory),
			                          entry->d_name,
			                          O_DIRECTORY);
			if (subdirectory == -1) {
			...
			leer_directorios(subdirectory_pointer,
			                 entry_path,
			                 cadena_a_buscar,
			                 funcion_a_comparar);
